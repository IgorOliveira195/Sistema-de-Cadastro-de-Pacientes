...
export default {
  ...
  getters: {
    ...
    deleteMessage: (state) => {
      ...
      return `Deseja excluir o paciente "${state.deleteTarget.name}"?`
    }
  },
  ...
  mutations: {
    ...
    SET_ADDRESS_OPTIONS (state, options) {
  ...
  actions: {
    async fetchList ({ state, commit }) {
      ...
      try {
        const { search, gender, sortBy, sortDir, page, perPage } = state.filters
        ...
        commit('SET_ERROR', 'Não foi possível carregar os pacientes.')
      }
    },

    async fetchAddressOptions ({ commit }) {
      try {
        ...
        commit('SET_ADDRESS_OPTIONS', data.data)
      } catch {
    ...
    async fetchOne ({ commit }, id) {
      ...
      try {
        ...
        commit('SET_ERROR', 'Não foi possível carregar o paciente.')
    ...
    async create ({ commit }, payload) {
      ...
      try {
        ...
        const message = extractApiError(error, 'Não foi possível salvar o paciente.')
    ...
    async remove ({ commit, dispatch, state }) {
      ...
      try {
        await patientService.remove(state.deleteTarget.id)
        commit('SET_DELETE_TARGET', null)
        await dispatch('fetchList')
      } catch {
        commit('SET_ERROR', 'Não foi possível excluir o paciente.')
        commit('SET_DELETE_TARGET', null)
      }
    },
